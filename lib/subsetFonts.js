...
function groupTextsByFontFamilyProps(
  ...
  return _.map(textsByFontUrl, (textsPropsArray, fontUrl) => {
    ...
    const fontVariationSettings = new Set(
      textsPropsArray.flatMap((obj) => obj.fontVariationSettings || [])
    );
...
const standardVariationAxes = new Set(['ITAL', 'WGHT', 'ITAL', 'SLNT', 'OPSZ']);

function warnAboutUnusedCustomVariationAxes(
  htmlOrSvgAssetTextsWithProps,
  assetGraph
) {
  const seenAxesByFontUrl = new Map();

  for (const { fontUsages } of htmlOrSvgAssetTextsWithProps) {
    for (const { fontUrl, fontVariationSettings } of fontUsages) {
      let seenAxes = seenAxesByFontUrl.get(fontUrl);
      if (!seenAxes) {
        seenAxes = new Set();
        seenAxesByFontUrl.set(fontUrl, seenAxes);
      }

      for (const fontVariationSettingsValue of fontVariationSettings) {
        for (const token of fontVariationSettingsValue.split(/,/)) {
          const matchToken = token.match(/^\s*((['"])\w+\2)/);
          if (matchToken) {
            seenAxes.add(unquote(matchToken[1]).toUpperCase());
          }
        }
      }
    }
  ...
  const unusedVariationAxesWarnings = [];
  for (const [fontUrl, seenAxes] of seenAxesByFontUrl.entries()) {
    let font;
    try {
    ...
    const unusedVariationAxes = Object.keys(font.variationAxes)
      .map((axis) => axis.toUpperCase())
      .filter(
        (axis) => !standardVariationAxes.has(axis) && !seenAxes.has(axis)
      );
    if (unusedVariationAxes.length > 0) {
      unusedVariationAxesWarnings.push({ fontUrl, unusedVariationAxes });
    }
  }

  if (unusedVariationAxesWarnings.length > 0) {
    const errorLog = unusedVariationAxesWarnings.map(
      ({ fontUrl, unusedVariationAxes }) =>
        `${fontUrl}: ${unusedVariationAxes.join(', ')}`
    );

    const message = `Unused custom variation axes detected in your variable fonts.
The below variable fonts contain custom axes that do not appear to be used on any of your pages.
This bloats your fonts and also the subset fonts that subfont creates.
Consider removing the unused axes using a tool like Slice <https://slice-gui.netlify.app/>`;
