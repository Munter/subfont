  ...
  if (fontVariationEntries.length > 0 && seenAxisValuesByAxisName) {
    for (const [
      ...
      if (seenAxisValues && seenAxisValues.size === 1) {
        variationAxes[axisName] = Math.min(
          Math.max([...seenAxisValues][0], min),
          max,
        );
        numAxesPinned += 1;
    ...
    for (const {
      ...
      for (const fontWeight of fontWeights) {
        noteUsedValue(
          ...
          'wght',
          Math.min(
            Math.max(fontWeight, minMaxFontWeight[0]),
            minMaxFontWeight[1],
          ),
        );
      }
      ...
      for (const fontStrech of fontStretches) {
        noteUsedValue(
          ...
          'wdth',
          Math.min(
            Math.max(fontStrech, minMaxFontStretch[0]),
            minMaxFontStretch[1],
          ),
        );
      }
    ...
    for (const [i, fontRelation] of cssAsset.outgoingRelations.entries()) {
      ...
      if (
        ...
        htmlOrSvgAssetTextsWithProps.every(({ fontUsages }) =>
          fontUsages.some(
            ({ fontUrl, pageText }) =>
              pageText && fontUrl === fontUsage.fontUrl,
          ),
        )
      ) {
