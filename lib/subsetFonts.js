...
function getHashedFileNameForFontAsset(fontAsset, fontRelation) {
...
These glyphs are used on your site, but they don't exist in the font you applied to them:`;
  ...
  for (const {
    ...
    for (const [i, fontRelation] of cssAsset.outgoingRelations.entries()) {
      ...
      // Keep the font inline if it's used on every page and we're only providing the subsets in one format:
      if (
        formats.length === 1 &&
        fontAsset.isInline &&
      ...
      if (fontAsset.isInline) {
        const fontAssetUrl =
          subsetUrl + getHashedFileNameForFontAsset(fontAsset, fontRelation);
    ...
    if (jsPreload) {
      ...
      cssAsset.parseTree.walkAtRules('font-face', (rule) => {
        ...
        rule.walkDecls(({ prop, value }) => {
          ...
          if (propName === 'font-family') {
            ...
            if (fontRelations.some(({ hrefType }) => hrefType === 'inline')) {
              return;
            }
      ...
      for (const [
        ...
        potentiallyOrphanedAssets.add(relation.to);
        relation.to = cssAsset.outgoingRelations.filter(
          ({ hrefType }) => hrefType !== 'inline'
        )[idx].to;
        relation.hrefType = 'rootRelative';
        relation.refreshHref();
      }
