...
const { parse, stringify } = require('font-family-papandreou');
...
function getPreferredFontUrl(cssFontFaceSrcRelations = []) {
  ...
  const relation = cssFontFaceSrcRelations.find(
    r =>
      (r.format && formatOrder.includes(r.format.toLowerCase())) ||
      typeOrder.includes(r.to.type)
  );
...
function groupTextsByFontFamilyProps(
  ...
  const snappedTexts = _.flatMapDeep(textByPropsArray, textAndProps => {
    ...
    const families = parse(family).filter(family =>
      availableFontFaceDeclarations.some(
        fontFace =>
          fontFace['font-family'].toLowerCase() === family.toLowerCase()
      )
    );
    const activeFontFaceDeclarations = _.compact(
      families.map(family =>
        fontSnapper(availableFontFaceDeclarations, {
          ...textAndProps.props,
          'font-family': stringify([family])
        })
      )
    );

    return activeFontFaceDeclarations.map(({ relations, ...props }) => {
  ...
  return _.map(textsByFontUrl, (textsPropsArray, fontUrl) => {
    ...
    const minRelation = _.minBy(
      textsPropsArray[0].fontRelations.filter(r => r.to.isLoaded),
      r => r.to.rawSrc.length
    );
    const smallestOriginalSize = minRelation.to.rawSrc.length;
    const smallestOriginalFormat = minRelation.to.type.toLowerCase();
...
async function getSubsetsForFontUsage(
  ...
  const allFontUsages = _.flatMap(
    htmlAssetTextsWithProps,
    asset => asset.fontUsages
  );

  try {
    const subsetLocalFont = require('./subsetLocalFont');
    const allFonts = _.uniq(
      _.compact(allFontUsages.map(fontUsage => fontUsage.fontUrl))
    );
    ...
    for (const fontUsage of allFontUsages) {
    ...
    const validFontUsages = allFontUsages.filter(
      fontUsage => fontUsage.fontUrl
    );
    const fontAssets = validFontUsages.map(
      fontUsage => assetGraph.findAssets({ url: fontUsage.fontUrl })[0]
    );
    const localFonts = fontAssets.filter(
      asset => asset.hostname !== 'fonts.gstatic.com'
    );
    for (const [fontUsage, fontAsset] of _.zip(validFontUsages, fontAssets)) {
    ...
    await Promise.all(
      formats.map(format => {
        ...
        return assetGraphForLoadingFonts.loadAssets(Object.values(formatUrls));
      })
    );
    ...
    for (const fontUsage of allFontUsages) {
      for (const format of formats) {
        ...
        if (!fontAsset.isLoaded) continue;
        if (!fontUsage.subsets) {
          fontUsage.subsets = {};
        }
...
function getFontFaceForFontUsage(fontUsage) {
  ...
  return [
    '@font-face {',
    ...Object.entries(fontUsage.props)
      .sort()
      .map(([prop, value]) => {
        if (prop === 'font-family') {
          return `${prop}: ${cssQuoteIfNecessary(`${value}__subset`)};`;
        }
...
function getUnusedVariantsStylesheet(
  ...
  return accumulatedFontFaceDeclarations
    ...
    .map(props => {
      ...
      return (
        `@font-face {` +
        `font-family:${props['font-family']}__subset;` +
...
async function subsetFonts(
  ...
  const htmlAssetTextsWithPropsPromise = Promise.all(
    htmlAssets.map(htmlProcess)
  ).finally(async () => {
    if (headlessBrowser) {
      await headlessBrowser.close();
    }
  });
  const htmlAssetTextsWithProps = _.compact(
    await htmlAssetTextsWithPropsPromise
  );
  ...
  for (const fontUsage of allFontUsages) {
    ...
    if (!originalFont.isLoaded) continue;
    try {
    ...
    for (const fontUsage of fontUsages) {
      ...
      for (const char of fontUsage.pageText) {
        ...
        if (charIdx === -1) {
          ...
          const position = new LinesAndColumns(htmlAsset.text).locationForIndex(
            charIdx
          );
...
These glyphs are used on your site, but they don't exist in the font you applied to them:`;
  ...
  for (const {
    ...
    for (const fontUsage of fontUsages) {
      removePreloadHint(htmlAsset, fontUsage);
    }
    ...
    if (unsubsettedFontUsages.length > 0) {
      ...
      insertPreloadLink(unsubsettedFontUsages, htmlAsset, insertionPoint);
    }
    ...
    if (!inlineFonts) {
      insertSubsetWithoutInlineFonts(cssAsset);
    }
    ...
    if (!inlineFonts) {
      for (const fontRelation of cssAsset.outgoingRelations) {
        ...
        if (fontAsset.contentType !== 'font/woff2') continue;
    ...
    if (!inlineFonts) {
      ...
      cssAssetInsertion = insertJsPreloading(cssAsset, htmlAsset, cssRelation);
    }
  ...
  for (const htmlAsset of htmlAssets) {
    lazyLoadLocalFontFace(htmlAsset);
  }
  ...
  for (const googleFontStylesheet of googleFontStylesheets) {
    lazyLoadGoogleFontFace(googleFontStylesheet);
  }
  ...
  for (const { fontFamilies, props } of allFontUsages.filter(
    fu => fu.subsets
  )) {
  ...
  for (const cssAsset of cssAssets) {
    ...
    cssAsset.eachRuleInParseTree(cssRule => {
      ...
      if (propName !== 'font' && propName !== 'font-family') return;

      if (cssRule.value.includes('var(')) {
        changesMadeToCustomPropertyDefinitions = cssProcessWithVar(cssRule);
      } else if (propName === 'font-family') {
        isChanged = cssProcessFontFamily(cssRule);
      } else if (propName === 'font') {
        isChanged = cssProcessFont(cssRule, isChanged);
      }
    });
    if (isChanged) {
      cssAsset.markDirty();
  ...
  await Promise.all(
    Array.from(subsetFontsToBeMinified).map(cssAsset => cssAsset.minify())
  );
  ...
  function lazyLoadGoogleFontFace(googleFontStylesheet) {
  ...
  function lazyLoadLocalFontFace(htmlAsset) {
  ...
  function insertPreloadLink(unsubsettedFontUsages, htmlAsset, insertionPoint) {
  ...
  function removePreloadHint(htmlAsset, fontUsage) {
  ...
  function insertJsPreloading(cssAsset, htmlAsset, cssRelation) {
    ...
    return jsPreloadRelation;
  }

  function insertSubsetWithoutInlineFonts(cssAsset) {
    for (const fontRelation of cssAsset.outgoingRelations) {
      const fontAsset = fontRelation.to;
  ...
  async function htmlProcess(htmlAsset) {
    ...
    assetGraph.eachAssetPreOrder(htmlAsset, traversalRelationQuery, asset => {
      if (asset.type !== 'Css' || !asset.isLoaded) return;
      ...
      for (const fontRelation of fontRelations) {
        ...
        node.walkDecls(({ prop, value }) => {
          const propName = prop.toLowerCase();
          if (propName === 'font-family') {
            fontFaceDeclaration[propName] = unquote(value);
          } else {
            fontFaceDeclaration[propName] = value;
          }
        });
    ...
    if (accumulatedFontFaceDeclarations.length > 0) {
      ...
      if (omitFallbacks) {
        ...
        for (const relation of _.flatMap(
          accumulatedFontFaceDeclarations,
          decl => decl.relations
        )) {
          potentiallyOrphanedAssets.add(relation.to);
  ...
  function cssProcessFont(cssRule) {
    ...
    }${lineHeightSuffix} ${fontFamilies.map(cssQuoteIfNecessary).join(', ')}`;
    return true;
  }

  function cssProcessFontFamily(cssRule) {
    let isChanged = false;
    const fontFamilies = cssListHelpers.splitByCommas(cssRule.value);
    const subsetFontFamilies = [];
    for (const fontFamily of fontFamilies) {
      const subsetFontFamily =
        webfontNameMap[unquote(fontFamily).toLowerCase()];
      if (!subsetFontFamily || fontFamilies.includes(subsetFontFamily)) {
        subsetFontFamilies.push(fontFamily);
      } else {
        isChanged = true;
        const quotedsubset = cssQuoteIfNecessary(subsetFontFamily);
        if (omitFallbacks) {
          subsetFontFamilies.push(quotedsubset);
        } else {
          subsetFontFamilies.push(quotedsubset, fontFamily);
        }
      }
    }
    cssRule.value = subsetFontFamilies.join(', ');
    return isChanged;
  }

  function cssProcessWithVar(cssRule) {
    let changesMadeToCustomPropertyDefinitions = false;
    const customPropertyDefinitions = findCustomPropertyDefinitions(cssAssets);
    ...
    return changesMadeToCustomPropertyDefinitions;
  }
}
